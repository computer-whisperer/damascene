...
**One `App` implementation, six targets.** As of August 2026 the same application code runs verified on Linux, Windows, macOS, Android, iOS, and the browser — touch, the soft keyboard, and screen-reader accessibility included on the platforms that have them.
...
|---|---|---|
| Linux / Windows / macOS desktop | `damascene-winit-wgpu` | Supported — batteries-included host; Linux/Wayland is the primary development platform; macOS and Windows verified August 2026 |
...
| Android | `damascene-android` | Supported — NativeActivity wrapper over the winit host (clipboard, IME, safe area wired); TalkBack and accessibility settings verified on a device, August 2026 |
...
| Accessibility (AccessKit) | `El` carries a semantic layer — `Role`, `aria_*` builders, live regions — lowered to an AccessKit tree with action routing (focus, click, scroll-into-view, text selection) on desktop, Android (delegate injected into `NativeActivity`, TalkBack-verified on a device), and iOS (`accesskit_ios`, Accessibility Inspector-verified). Screen-reader text editing speaks the full AT text protocol: synthesized `TextRun`s, caret + selection reporting, and `SetTextSelection` / `ReplaceSelectedText` routed back through `on_event`. `App::drain_announcements` feeds a live region; `prefers-reduced-motion` / color-scheme / contrast preferences surface through `BuildCx`; four accessibility lints (missing names, missing alt text, low contrast, undersized targets) run in the same bundle pass as the layout lints. |
| Touch + gestures | multi-touch contact registry routes the primary contact through the pointer model; two-finger pinch drives per-axis plot zoom, viewport zoom, and camera pan + dolly; long-press raises tooltips; touch-sized multi-click windows; macOS trackpad pinch and browser ctrl+wheel map to the same gesture. `docs/TOUCH_PLAN.md` is the gesture-grammar note. |
| Mobile ergonomics | hosts report safe-area insets (`RunnerCore::set_safe_area`) and placement keeps popovers, menus, and tooltips inside the notch / home-indicator keepouts; the soft keyboard is a runtime-owned inset (`interactive-widget=resizes-content` semantics) — layout shrinks by the keyboard band and the focused field is revealed above it (iOS keyboard-frame observer, web `visualViewport`); an oversize dropdown flips, shrinks to the roomier side, and scrolls (Floating-UI-style placement). |
...
| 2D plots | line / scatter / step marks and stacked lane plots from a builder spec (`plot::line` / `scatter` + `PlotSpec`); scientific navigation — box-zoom, per-axis drag and wheel, autoscale — with a multi-series cursor readout and legend; linear / log / time scales, epoch-relative timestamps for jitter-free live data; on touch, drag pans, tap places the crosshair, pinch zooms per axis, and embedded plots yield vertical swipes to the enclosing scroll. `cargo run -p damascene-examples --bin plot`; design note in `docs/PLOT2D_PLAN.md`. |
...
| App-supplied SVG icons | `icon(SvgIcon::parse_current_color(include_str!("path.svg")))` paints any `currentColor` SVG through the same icon pipeline used for built-in `IconName`s; `SvgIcon::parse` keeps full-color art as authored — solid paints, linear/radial gradients, and `clip-path` regions included (the project logo is the fixture). `icon_button(source)` wraps either with the standard interactive surface. Unknown string-typed icon names render an empty box rather than panicking. |
...
cargo test --workspace --lib                          # ~1,870 unit tests
```
