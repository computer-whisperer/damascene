//! Native Vulkan backend for custom Damascene hosts.
//!
...
//! validating backend parity or embedding Damascene into an existing Vulkan
//! renderer built on `vulkano`.
//!
//! The public entry point is [`Runner`]. Its surface mirrors
//! `damascene-wgpu::Runner` where the GPU APIs allow it: the host owns the
//! window, device, queue, swapchain, and event loop; the runner owns
//! Damascene interaction state, layout/draw-op preparation, Vulkan
