...
//! the vulkano backend. Mirrors `damascene-wgpu/src/scene.rs`.
//!
...
//! [`damascene_core::scene::gpu`], shared with every backend. Only the GPU
...
/// damascene host waits on each frame's fence before the next `prepare`, so the
...
impl Scene3DPaint {
    ...
    /// after the previous frame's fence has signalled (the damascene host waits on
...
// ---- vertex input layouts (offsets mirror damascene_core::scene::gpu) ----
