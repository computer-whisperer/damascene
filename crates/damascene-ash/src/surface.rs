...
//!
//! The host owns the Vulkan image and image view. `damascene-ash` only
//! caches descriptor sets that sample the supplied view while recording
//! Damascene's normal paint stream.
...
impl SurfacePaint {
    pub(crate) fn new(
        ...
        let instance_buf = GpuBuffer::new(
            device,
            allocator,
            "damascene_ash::surface_instances",
    ...
    fn ensure_descriptor(&mut self, device: &ash::Device, texture: &AppTexture) -> Result<usize> {
        let backend = texture
            ...
            .unwrap_or_else(|| {
                panic!(
                    "damascene-ash expected AshAppTexture, got {}",
...
///
/// `damascene-ash` cannot inspect raw ash image creation flags, so this
...
/// `SHADER_READ_ONLY_OPTIMAL` when Damascene samples it; use
...
/// make sure the image is actually in that layout when Damascene draw
/// commands execute.
pub fn app_texture_with_layout(
    ...
    let surface_format = match format {
        ...
        _ => panic!("unsupported damascene-ash app texture format: {format:?}"),
    };
    assert!(
        extent.width > 0 && extent.height > 0,
        "damascene-ash app textures must have non-zero extent"
    );
