    ...
    scroll([column([
        ...
        section_card(
            ...
            "`surface(AppTexture)` composites pixels the app writes each frame — \
             ...
             Damascene samples it across each tile's resolved rect with bilinear \
    ...
    let Some(tex) = tex else {
        return paragraph(
            "This demo requires a host that allocates an `AppTexture` and \
             ...
             or `crates/damascene-ash-demo/src/lib.rs`). \
...
fn animated_surface_cell(label: &str, backdrop: Color, tex: AppTexture, alpha: SurfaceAlpha) -> El {
    ...
    column([
        ...
        stack([
            ...
            // while this tile is visible, Damascene asks the host for a
