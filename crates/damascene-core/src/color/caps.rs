...
//!
//! Damascene apps run on a wide range of hosts — bare X11, plain Wayland
...
/// Mirrors the protocol's `render_intent` enum. Damascene always requests
...
impl ColorPreferences {
    ...
    /// pass skip it (the `damascene-wgpu` host does, today). `BT2020_LINEAR`
