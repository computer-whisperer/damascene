...
pub(crate) struct LayoutOverrides {
    ...
    /// CSS `flex-direction` → Aetna [`Axis`]. Only consulted when
    /// `display_is_flex == Some(true)`.
    pub flex_axis: Option<Axis>,
    /// CSS `align-items` → Aetna [`Align`].
    pub align: Option<Align>,
    /// CSS `justify-content` → Aetna [`Justify`].
...
/// Reduced CSS `overflow` model: only the values that map onto Aetna
/// primitives (`clip()` and `scroll([...])`). `visible` and `clip`
/// (the CSS keyword, not Aetna's `.clip()` modifier) collapse to
...
pub(crate) struct ComputedStyle {
    ...
    /// dropped because Aetna doesn't expose per-element font-family
    ...
    /// scalar. Offset / spread / color are dropped because Aetna's
...
fn apply_declaration(style: &mut ComputedStyle, prop: &str, value: &str, lints: &Lints) {
    match prop {
        ...
        "font-family" => {
            ...
            match classify_font_family(value) {
                ...
                FontFamilyClass::Other(name) => lints.push(
                    ...
                    format!("font-family: {name} (only monospace families map onto Aetna)"),
                ),
        ...
        "align-items" => match value.to_ascii_lowercase().as_str() {
            ...
            "baseline" => lints.push(
                FindingKind::DroppedDeclaration,
                "align-items: baseline (Aetna has no baseline alignment)",
            ),
            _ => {}
        },
        ...
        "float" => match value.to_ascii_lowercase().as_str() {
            ...
            other => lints.push(
                FindingKind::DroppedDeclaration,
                format!("float: {other} (no float layout in Aetna)"),
            ),
        },
...
/// Aetna's `shadow(f32)` modifier doesn't carry them. CSS multi-shadow
