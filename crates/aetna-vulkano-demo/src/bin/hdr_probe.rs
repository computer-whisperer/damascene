...
//!
//! Validates the *compliant* HDR-output path that `aetna-winit-wgpu`
...
//! `aetna-winit-vulkano` host crate. niri is SDR-only, so it will show the
...
impl ApplicationHandler for Host {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attrs = Window::default_attributes()
            .with_title("aetna HDR probe — left: SDR white | right: HDR white")
